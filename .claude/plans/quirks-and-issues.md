...
**On `next/image`**: only a partial fit. Its `priority` prop (`fetchpriority="high"` + preload, skips lazy-loading) is the right tool for genuinely above-the-fold images — worth using on just the first row for a quick LCP win — but for non-priority images it falls back to the same native `loading="lazy"` this app already uses, so it doesn't add anything toward the actual "boost priority as an image approaches the viewport while scrolling" idea; that still needs custom `IntersectionObserver` logic regardless of which image tag is used. It would also require wiring a custom `loader` against `/api/storage/[babyId]/[...path]`, since this app deliberately avoids handing out raw/optimizer URLs (see `security-review.md` #3) — not a drop-in swap.
