...
export default function InventoryItemModal({
  ...
  return (
    <div>
      ...
      {open && (
        <Modal onClose={() => setOpen(false)} title={<>{isEditMode ? <Pencil className="h-4.5 w-4.5 text-primary" /> : <Package className="h-4.5 w-4.5 text-primary" />} {isEditMode ? t('editTitle') : t('addTitle')}</>}>
          {/* Modal Content */}
