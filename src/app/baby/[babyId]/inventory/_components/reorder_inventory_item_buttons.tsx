...
import { reorderInventoryItem } from "@/utils/actions/inventory";

export default function ReorderInventoryItemButtons({
  babyId,
  itemId,
  isFirst,
  isLast,
  ...
  const handleMove = async (direction: "up" | "down") => {
    ...
    try {
      await reorderInventoryItem(babyId, itemId, direction);
      router.refresh();
    } catch (err) {
      console.error(err);
